...
///
/// ffmpeg/ffprobe come from the bundled dependency container (same path
/// production uses).  The fixture calls <see cref="ContainerBuilder.EnsureReadyAsync"/>
/// before generating any clips, which auto-builds the image on first run.
/// First-time cost on a fresh machine is the same as production: 10–20 min
/// to build the container; subsequent test runs reuse the image.
///
...
public static class TestVideoFixture
{
    ...
    public static async Task BuildClips()
    {
        ...
        // All tool invocations route through the dependency container —
        // tests must not pick up host ffmpeg/ffprobe.  EnsureReadyAsync
        // auto-builds the image on first use (10–20 min cold), reuses it
        // afterward.  TempDir is bind-mounted so the container can read
        // and write our test artifacts.
        await ContainerBuilder.EnsureReadyAsync(mounts: [TempDir], ct: CancellationToken.None);
    ...
    /// goes through <see cref="ContainerTools"/>, configured by
    ...
    {
        (int code, string _, string err) = await ContainerTools.RunFfmpegAsync(args, CancellationToken.None);
        if (code != 0)
